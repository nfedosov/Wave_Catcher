function [sensor_waves] = bi_blob_on_sensor(cortex,  PARAMS, G,strt,n)
...
% 
    ...
    for i = t
       wave((i+1),:) = (cos(2*pi * (n) / nspoints))*cos(2*pi*i/ntpoints);
    end%wave - t (0 ... T) x 
    ...
    dist_from_source = 0.05; % 5 cm
    sigma = 0.01; % 95% - 2 cm
    
    
    
    dist = (vertices(strt,:)-vertices);
    prob_unnorm = exp(-0.5*((dist-dist_from_source)/sigma).^2);
    prob = prob_unnorm/sum(prob_unnorm);
    
    
    strt = find(cumsum(prob) >= rand(), 1);
...
%                 end
    ...
    for i = t
       wave((i+1),:) = (cos(2*pi * (n) / nspoints))*sin(2*pi*i/ntpoints);
    end%wave - t (0 ... T) x 
...
%     end
    ...
    % waves on sensors  
            ...
            for t = 1:ntpoints
                ...
                sensor_waves(:,t) = sensor_waves(:,t) + FM_s*wave(t,:)';
            end
